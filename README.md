...
- 공개 채팅과 1:1 DM을 SQLite에 영구 저장합니다. DM 본문과 첨부 파일은 대화 당사자만 조회할 수 있습니다.
- 최초 접속 시 공개 채팅 최근 50개와 DM 상대별 최근 30개를 불러옵니다. 기록이 더 있으면 `↑ 이전 메시지 더 불러오기`로 50개씩 추가합니다.
- 전체 채팅에서 `@`를 입력하면 활성 사용자 자동완성이 열립니다. 온라인/오프라인 상태를 확인하고 방향키, Enter 또는 Tab으로 선택할 수 있습니다.
- 나를 멘션하거나 내가 보낸 공개 메시지에 답장한 메시지는 행 배경과 포인트 바로 강조하고 토스트로 알립니다.
