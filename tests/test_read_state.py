...
class TestReadStateApi:
    ...
    def test_ack_dm_read_state_by_numeric_username(self):
        client_alice, user_alice = session_client("alice")
        client_qpt, user_qpt = session_client("20011225")

        dm1 = database.save_direct_message(user_qpt, user_alice, "Chatqpt says hi")
        raw_dm1 = int(dm1["message_id"].replace("dm:", ""))

        dm_key = f"dm:{user_qpt['id']}"
        resp = client_alice.get("/api/read-states", headers=ORIGIN)
        assert resp.json()["unread_counts"].get(dm_key) == 1

        ack_resp = client_alice.post("/api/read-states/ack", headers=ORIGIN, json={
            "conversation_type": "dm",
            "conversation_id": "20011225",
            "last_read_message_id": raw_dm1,
        })
        assert ack_resp.status_code == 200
        assert ack_resp.json()["unread_counts"].get(dm_key, 0) == 0
